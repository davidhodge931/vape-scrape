...
# for vapo we wantr 4 slashes for products
urls <- urls |>
...
get_html_with_retry <- function(url, retries = 3, delay = 5 ) {
  ...
  while(attempt <= retries) { # loop for number of iterations set in retries
    ...
    try({
      ...
      Sys.sleep(5)  # Polite delay as per: print(bow("https://www.vapo.co.nz/"))
      
      
      #testing elements
      #image <- url_html_live %>% html_elements("div.swiper-slide img") |>
      # html_attr("src")
      # image_url <- image[1]
      
      # image <- wait_for_elements(url_html_live, "div.carousel2-imageContainer-Xbc img") |>
      image <- wait_for_elements(url_html_live, "div.swiper-slide img") |>
        html_attr("src")
      # image_url <- image[grepl("^https?://", image)][1] #get the first URL from the extracted html elements
      image_url <- paste0("https:",image[1]) #get the first URL from the extracted html elements
